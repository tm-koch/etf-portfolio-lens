...
The published site is an installable PWA named `ETF Porfolio Lens`. It requires HTTPS (GitHub Pages satisfies this) and a first online visit to cache the application shell and any data needed offline. The PWA assets are published by `scripts/publish-gh-pages.ps1`; validate installation, offline startup, cached catalog and snapshot loading, PDF import, sharing, and responsive Android layouts before packaging a future Trusted Web Activity. Native Android project creation and Play Store submission are outside this change.
