...
The Portfolio tab SHALL provide an accessible share action that serializes the current ETF positions and share counts into a versioned URL fragment without requiring a server-side share service.

#### Scenario: Share a populated portfolio

- **WHEN** the user activates the share action with one or more selected positions
- **THEN** the application creates a URL containing a versioned encoded representation of every selected position's ISIN and share count
...
The application SHALL inspect the URL for a supported portfolio share payload during startup and SHALL apply a valid linked portfolio before falling back to the recipient's locally stored portfolio.
...
- **THEN** the linked positions and share counts become `state.portfolio`, are persisted to local storage, and the Portfolio tab is selected
...
The application SHALL accept only supported payload versions containing a duplicate-free array of positions with non-empty string ISINs and finite non-negative share counts, and SHALL treat malformed payloads as non-fatal.
...
- **WHEN** a decoded payload contains missing identifiers, invalid share counts, duplicate positions, or an invalid portfolio structure
