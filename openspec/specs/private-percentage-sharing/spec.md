# private-percentage-sharing Specification

## Purpose
TBD - created by archiving change private-percentage-sharing. Update Purpose after archive.
## Requirements
...
The application SHALL use the existing selected-position table and Shares inputs for private portfolios and SHALL communicate that the values are relative weighting units through the portfolio hint or share feedback. It SHALL hide the valuation basis control and the portfolio-level valuation-status box for percentage-only portfolios. It SHALL omit Price and Value CHF headers and cells entirely, SHALL display each position's normalized relative Weight, and SHALL retain the Remove control. It SHALL NOT resolve absolute position valuations while rendering percentage-only rows.
