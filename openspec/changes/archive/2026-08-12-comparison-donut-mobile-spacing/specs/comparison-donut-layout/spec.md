...
#### Scenario: Smartphone layout reduces empty vertical space
- **WHEN** the comparison view is rendered on a smartphone-sized viewport
- **THEN** the donut layout SHALL reduce unused vertical padding above and below the donut compared with the desktop layout
- **AND** the chart SHALL remain readable without overlapping the legend
